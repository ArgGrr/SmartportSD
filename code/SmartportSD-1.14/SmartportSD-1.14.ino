...
void setup() {
  ...
  digitalWrite(OLEDsck, 0);
  digitalWrite(OLEDsda, 0);      
  u8x8.begin();
  u8x8.setPowerSave(0);
  u8x8.setFont(u8x8_font_chroma48medium8_r);
  strcpy_P(chrdata, (char*)pgm_read_word(&(string_table[0])));
  u8x8.drawString(0,0,chrdata);
  strcpy_P(chrdata, (char*)pgm_read_word(&(string_table[1])));
...
void loop() {
  ...
  while(1) {
    ...
    {
      strcpy_P(chrdata, (char*)pgm_read_word(&(string_table[2])));
...
#endif
    strcpy_P(chrdata, (char*)pgm_read_word(&(string_table[3])));
    u8x8.drawString(0,3,chrdata);
    Serial.print(F("\r\nError init card"));  
    led_err();
    return 1;
...
void led_err(void)
{
    ...
    strcpy_P(chrdata, (char*)pgm_read_word(&(string_table[4])));
    u8x8.drawString(0,3,chrdata);
